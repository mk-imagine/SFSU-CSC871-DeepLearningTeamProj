...
"""
Build Mixed Style Transfer Model
"""
def build_model(model: nn.Sequential,
                content_img: torch.Tensor,
                style_img: torch.Tensor,
                secondStyle_img_t: torch.Tensor,
    ...
    for layer in pretrained_cnn.children():
        ...
        if name in style_layers:
            ...
            # second style image
            secondStyle_losses
            target_s = model(secondStyle_img_t).detach()
            if isinstance(s_loss_func, SlicedWassersteinLoss):
                secondStyle_loss = s_loss_func(target_s, kwargs["scalar"], kwargs["proj_n"])
            else:
                secondStyle_loss = s_loss_func(target_s)
            model.add_module("s2_loss_{}".format(conv2d_count), secondStyle_loss)
            secondStyle_losses.append(secondStyle_loss)
    ...
    return model, content_losses, style_losses, secondStyle_losses


"""
Generate mixed blended style image
"""
def generate_image(model: nn.Sequential,
                   content_losses: list[float],
                   style_losses: list[float],
                   secondStyle_losses: list[float],
    ...
    while run[0] <= epochs:
        ...
        def closure():
            ...
            for sl2 in secondStyle_losses:
                style2_score += sl2.loss

            style_score = (crossfader * style1_score) + ((1-crossfader) * style2_score)
...
def build_model_and_generate_image(model: nn.Sequential,
    ...
    print("Building Mixed Style Transfer Model")
    model, content_losses, style_losses, secondStyle_losses = build_model(
        model, content_img, style_img, secondStyle_img_t, c_loss_func, s_loss_func, content_layers, 
        style_layers, model_normalization, **kwargs
    )

    return generate_image(model, content_losses, style_losses, secondStyle_losses, output_img, optimizer,
                          epochs, content_weight, style_weight, crossfader)
...
if __name__ == "__main__":
    ...
    secondStyle_img_t = load_image(Path(style_image_dir, secondStyle_img), (img_size, img_size))
    ...
    assert secondStyle_img_t.size() == content_img_t.size(), "Second style and content images must be the same size"
